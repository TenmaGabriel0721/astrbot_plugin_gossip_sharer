...
- **目标会话 LLM 唤醒**：通过 `wake_qq_session_task` 把任务作为目标 QQ 群聊或私聊事件投递给目标会话 LLM，走正常 AstrBot pipeline。
...
- **目标上下文注入**：转发成功后，把文字桥接信息与图片转发说明写入目标会话上下文，避免保存会失效的临时图片路径。
...
- 需要目标 LLM 结合目标会话上下文、工具和权限来处理时，用 `wake_qq_session_task`。
...
群聊目标会构造一条发送者为原请求者 QQ、并 @ Bot 的合成群消息；私聊目标会使用目标好友 QQ 构造正确的私聊会话路由，同时在任务内容中保留原请求者信息。后续均由 AstrBot 正常 waking/process/respond 流程处理，目标 LLM 的最终回复会自然发到对应群聊或私聊。
...
1. 先把文字/图片消息发送到目标会话。
2. 再把桥接说明写入目标会话上下文。
3. 图片本身不写入目标会话历史；历史只记录图片已随消息发送，避免临时文件清理、网络链接失效或 base64 长期占用上下文。
4. 需要让 Bot 再次理解图片时，在目标会话中引用那条图片消息；AstrBot 会从引用消息重新读取图片。

如果需要立即让目标会话 LLM 处理一项任务，使用 `wake_qq_session_task`；`send_cross_message` 只负责发送与上下文注入。
