...
- 来源 LLM 请求阶段提前保存当前和引用图片的内存快照，避免 `media_image_*` 临时文件在工具调用前被清理。
- 模型误用历史中的失效 AstrBot 临时图片路径时，如果本轮只有一张可用图片，会安全回退到本轮唯一短引用图片。
- 纯图片任务最终回复为空时，通过 Agent 完成钩子直接投递待发送附件，不再依赖可能被 AstrBot 跳过的发送后钩子。
- 目标任务明确说明选中附件由插件自动投递，禁止目标 LLM 搜索、替换或调用其他发送工具重复发送。
- 保留一次性附件发送标记，空回复兜底与正常回复后投递不会造成重复发送。
- GIF 使用双轨数据：原始动图保留给 QQ 投递，第一帧转为 PNG 供来源和目标 LLM 识别，避免 Gemini 因 `image/gif` MIME 拒绝请求。
- GIF 首帧转换失败时从 LLM 图片输入中移除不兼容动图，但不阻止原始 GIF 的真实发送。
